...
func fannkuch(idxMin int, ch chan bool) {
		...
		for i, idx := nFank-1, idxMin; i > 0; i-- {
				d := idx / Fact[i]
				count[i] = d
				idx = idx % Fact[i]

				copy(pp[:], p[:]) // One could copy only p[:nFank], but it is slower.
				for j := 0; j <= i; j++ {
						if j+d <= i {
								p[j] = pp[j+d]
						} else {
...
//export GoFannkuchRun
func GoFannkuchRun(num int) {
		// flag.Parse()
		// if flag.NArg() == 1 {
		...
		Fact = make([]int, nFank+1)
		Fact[0] = 1
		for i := 1; i < len(Fact); i++ {
				Fact[i] = Fact[i-1] * i
		}

		CHUNKSZ = (Fact[nFank] + NCHUNKS - 1) / NCHUNKS
		CHUNKSZ += CHUNKSZ % 2
		NTASKS = (Fact[nFank] + CHUNKSZ - 1) / CHUNKSZ
