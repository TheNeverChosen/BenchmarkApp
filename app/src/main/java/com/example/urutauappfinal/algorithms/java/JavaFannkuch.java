package com.example.urutauappfinal.algorithms.java;
...
public final class JavaFannkuch implements Runnable
{
    ...
    {
        n = num;
        if ( n < 0 || n > 13 ) {         // 13! won't fit into int
            printResult( n, -1, -1 );
            return;
        }
        if ( n <= 1 ) {
        ...
        for ( int i=0; i<nthreads; ++i ) {
            threads[i] = new Thread( new JavaFannkuch() );
            threads[i].start();
        }
