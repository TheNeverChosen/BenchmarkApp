...
// https://benchmarksgame-team.pages.debian.net/benchmarksgame/program/binarytrees-gpp-5.html
...
using MemoryPool = boost::container::pmr::monotonic_buffer_resource;
...
int run_parallel(unsigned depth, int iterations, unsigned int workers = std::thread::hardware_concurrency())
{
    ...
    for(unsigned i = 0; i < workers; ++i) {
        threads.push_back(std::thread([&counter, depth, &output] {
            boost::container::pmr::unsynchronized_pool_resource upperPool;
...
Java_com_example_urutauappfinal_algorithms_cpp_CppAlgorithms_cppBinaryTreesRun(JNIEnv *env, jclass thiz, jint num)
{
